...
def current_measurement():
    ...
    dut["EMeter"].setup_current_measurement(
        current_range=2e-10,  # Also 'MIN'/'MAX' e.g. 20e-12/12e-3 A or any number in between or None for autorange
        ...
        filter=("REP", 5),
    )  # Average filter to apply e.g. REPeat measurement 5 times andy yield average
    # Turn the output on
    dut["EMeter"].on()
