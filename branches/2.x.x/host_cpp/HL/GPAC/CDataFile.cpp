...
// not found.
float CDataFile::GetFloat(t_Str szKey, t_Str szSection, float def_val)
{
...
// not found.
int	CDataFile::GetInt(t_Str szKey, t_Str szSection, int def_val)
{
	t_Str szValue = GetValue(szKey, szSection);
...
// not found.
bool CDataFile::GetBool(t_Str szKey, t_Str szSection, bool def_val)
{
	bool bValue = false;
	t_Str szValue = GetValue(szKey, szSection);
		...
		|| CompareNoCase(szValue, "yes") )
	{
		...
		|| CompareNoCase(szValue, "false") 
		|| CompareNoCase(szValue, "no") )
	{
		bValue = false;
	}
	else bValue = def_val;
...
#ifdef WIN32
	OutputDebugStringA(szMsg.c_str());
#endif
