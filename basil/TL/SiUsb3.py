    ...
    def init(self):
        ...
        if self._init['board_id'] and int(self._init['board_id']) >= 0:
            ...
            if not devices:
                ...
                logger.info('Found USB board(s): {}'.format(', '.join(('%s with ID %s (FW %s)' % (device.board_name, "".join(filter(type(device.board_id).isdigit, device.board_id)), "".join(filter(type(device.fw_version).isdigit, device.fw_version)))) for device in devices)))
                if len(devices) > 1:
