...
module uart_master (
    ...
    always @* begin
        ...
        case (STATE)
            ...
            SET_LEN: begin
                if ((!uart_busy) && op_done) NEXTSTATE = ROGER;
                else NEXTSTATE = SET_LEN;
            end
            SET_ADD: begin
                if ((!uart_busy) && op_done) NEXTSTATE = ROGER;
                else NEXTSTATE = SET_ADD;
            end
            READ: begin
                if ((!uart_busy) && op_done) NEXTSTATE = ROGER;
                else NEXTSTATE = READ;
        ...
        if (UART_RST) begin
            ...
            case (STATE)
                ...
                READ: begin
                    ...
                    if (is_transmitting) transmit <= 0;
                    else if (cnt / 4 >= block_len) op_done <= 1;
