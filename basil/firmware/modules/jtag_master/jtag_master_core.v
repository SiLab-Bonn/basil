...
module jtag_master_core #(
    ...
    always @(posedge BUS_CLK) begin
        ...
        end else if (BUS_WR && BUS_ADD < 16) begin
    ...
    always @(posedge BUS_CLK) begin
        if (BUS_RD) begin
            ...
            else if (BUS_ADD == 13) BUS_DATA_OUT_REG <= {7'b0, CONF_EN};
            else if (BUS_ADD == 14) BUS_DATA_OUT_REG <= MEM_BYTES[7:0];
            else if (BUS_ADD == 15) BUS_DATA_OUT_REG <= MEM_BYTES[15:8];
            else if (BUS_ADD < 16) BUS_DATA_OUT_REG <= status_regs[BUS_ADD[3:0]];
        end
    end
    ...
    always @(*) begin
        ...
        else if (PREV_BUS_ADD < 16 + MEM_BYTES + MEM_BYTES) BUS_DATA_OUT = BUS_OUT_MEM;
    ...
    always @(posedge JTAG_CLK) begin
        if (RST_SYNC) state <= TEST_LOGIC_RESET;
    ...
    always @(*) begin
        case (state)
            ...
            RUN_TEST_IDLE: begin
                if (START_SYNC || (out_word_cnt != WORD_COUNT && SEN_INT)) begin
            ...
            SELECT_DR_SCAN: begin
                if (JTAG_OP_CODE == DR_SCAN) next_state <= CAPTURE_DR;
                else if (JTAG_OP_CODE == IR_SCAN) next_state <= SELECT_IR_SCAN;
                else next_state <= SELECT_IR_SCAN;
            end
            CAPTURE_DR:     next_state <= SHIFT_DR;
            SHIFT_DR: begin
                if (out_bit_cnt == STOP_BIT) next_state <= EXIT1_DR;
            ...
            SHIFT_IR: begin
                if (out_bit_cnt == STOP_BIT) next_state <= EXIT1_IR;
    ...
    always @(negedge JTAG_CLK) begin
        ...
        else if (state == SHIFT_DR || state == SHIFT_IR) out_bit_cnt <= out_bit_cnt + 1;
        else out_bit_cnt <= 0;
    end

    // Word counter
    always @(posedge JTAG_CLK) begin  // - 1 because we must change of step on last bit
        if (RST_SYNC) out_word_cnt <= 0;
        else if (state == UPDATE_DR || state == UPDATE_IR) out_word_cnt <= out_word_cnt + 1;
        else if (out_word_cnt == WORD_COUNT && state == RUN_TEST_IDLE) out_word_cnt <= 0;
    end

    // Reset counter
    always @(posedge JTAG_CLK) begin
        if (state == TEST_LOGIC_RESET) reset_cnt <= reset_cnt + 1;
        else reset_cnt <= 0;
    end
    ...
    always @(negedge JTAG_CLK) begin
        case (state)
            TEST_LOGIC_RESET: begin
                if (next_state == RUN_TEST_IDLE) TMS <= 0;
                else if (reset_cnt < 5) TMS <= 1;
                else TMS <= 0;
            end
            RUN_TEST_IDLE: begin
                if (next_state == SELECT_DR_SCAN) TMS <= 1;
                else TMS <= 0;
            end
            SELECT_DR_SCAN: begin
                if (next_state == CAPTURE_DR) TMS <= 0;
                else TMS <= 1;
            end
            CAPTURE_DR: begin
                if (next_state == SHIFT_DR) TMS <= 0;
                else TMS <= 1;
            end
            SHIFT_DR: begin
            ...
            SELECT_IR_SCAN: begin
                if (next_state == CAPTURE_IR) TMS <= 0;
                else TMS <= 1;
            end
            CAPTURE_IR: begin
                if (next_state == SHIFT_IR) TMS <= 0;
                else TMS <= 1;
            end
            SHIFT_IR: begin
                // -1 beacause we need to change state on last bit
    ...
    always @(posedge JTAG_CLK) SLD <= (sync_ld[1] == 1 && sync_ld[0] == 0);
    ...
    always @(negedge JTAG_CLK) TDI <= SDI_MEM & SEN_INT;

    always @(negedge JTAG_CLK) SEN <= SEN_INT;
    ///
