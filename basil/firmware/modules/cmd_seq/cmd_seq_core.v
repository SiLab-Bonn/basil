...
module cmd_seq_core #(
    ...
    always @(posedge BUS_CLK) begin
        ...
        end else if (BUS_WR && BUS_ADD < 14) status_regs[BUS_ADD[3:0]] <= BUS_DATA_IN;
    end
    ...
    always @(posedge BUS_CLK) begin
        if (BUS_RD) begin
            ...
            else if (BUS_ADD == 1) BUS_DATA_OUT <= {7'b0, CONF_READY};
            else if (BUS_ADD < 14) BUS_DATA_OUT <= status_regs[BUS_ADD[3:0]];
            else if (BUS_ADD < 16) BUS_DATA_OUT <= 8'b0;
            else if (BUS_ADD < (16 + CMD_MEM_SIZE)) BUS_DATA_OUT <= CMD_MEM_DATA;
        end
    end
    ...
    always @(posedge CMD_CLK_IN)
        ...
        else START_STOP_REPEAT_OK <= 1'b1;

    reg [31:0] SET_REPEAT_COUNT;
    always @(posedge CMD_CLK_IN)
        if (CONF_START_REPEAT_CMD_CLK + CONF_STOP_REPEAT_CMD_CLK == CONF_CMD_SIZE_CMD_CLK)
            SET_REPEAT_COUNT <= 1;
        else SET_REPEAT_COUNT <= CONF_REPEAT_COUNT_CMD_CLK;
    ...
    reg cmd_data_ser;
    always @(posedge CMD_CLK_IN) begin
        if (state == WAIT) cmd_data_ser <= 1'b0;
        else cmd_data_ser <= CMD_MEM_DATA_BUF[7-((cnt_buf)%8)];
    end
    ...
    // ready signal
    always @(posedge CMD_CLK_IN) CMD_READY <= ~next_state;
    ...
    reg CMD_BUSY_FLAG;
    always @(posedge CMD_CLK_IN)
        if (state != next_state && next_state == SEND) CMD_BUSY_FLAG <= 1'b1;
        else CMD_BUSY_FLAG <= 1'b0;

    // ready flag
    reg CMD_READY_FLAG;
    always @(posedge CMD_CLK_IN)
        if (state != next_state && next_state == WAIT) CMD_READY_FLAG <= 1'b1;
        else CMD_READY_FLAG <= 1'b0;
    ...
    always @(posedge BUS_CLK)
        if (RST) CONF_READY <= 1;
        else if (CMD_BUSY_FLAG_BUS_CLK || CONF_START) CONF_READY <= 0;
        else if (CMD_READY_FLAG_BUS_CLK) CONF_READY <= 1;
