...
module rec_sync #(
    ...
    always @(posedge FCLK) shift_reg <= {shift_reg[DSIZE-2:0], datain};
    ...
    always @(posedge FCLK)
        ...
        else bitslip_cnt <= {bitslip_cnt[DSIZE-2:0], bitslip_cnt[DSIZE-1]};

    reg [DSIZE-1:0] fdataout;
    always @(posedge FCLK)
        if (bitslip_cnt[0]) fdataout <= shift_reg;
        else fdataout <= fdataout;
    ...
    always @(state or wait_cnt or decoder_err) begin  // or data or old_data
        ...
        case (state)
            ...
            WAIT:
            if (wait_cnt == 2) next_state = CHECK;
            ...
            BITSHIFT: next_state = WAIT;

            IDLE:
            if (decoder_err == 1'b1) next_state = WAIT;
    ...
    begin
            ...
            case (next_state)
                ...
                WAIT: begin
                    if (decoder_err == 1'b1) rec_sync_ready <= 1'b0;
...
`ifdef SYNTHESIS_NOT
    ...
    chipscope_ila ichipscope_ila (
        ...
        .CLK(WCLK),
        .TRIG0({
            lck, eye_size, REC_SYNC_ERROR, data, BITSLIP_FLAG, state, pa_ready_flag, reset, pll_rst
        })
    );
`endif
