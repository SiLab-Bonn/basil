...
module sync_master (
    ...
    reg [3:0] data_bz2;
    always @(posedge clk) data_bz2[3:0] <= {data_bz2[2:0], bz2};
    assign bb0 = data_bz2[ctrlint];

    reg [3:0] data_cz2;
    always @(posedge clk) data_cz2[3:0] <= {data_cz2[2:0], cz2};
    assign cc0 = data_cz2[ctrlint];

    reg [3:0] data_dz2;
    always @(posedge clk) data_dz2[3:0] <= {data_dz2[2:0], dz2};
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (pipe_ce0) sdataout <= sdataa | sdatab | sdatac | sdatad;
    ...
    always @(posedge clk_2x) DDRQ_DLY[1:0] <= DDRQ[1:0];

    reg [3:0] DDRQ_DATA;
    always @(posedge clk_2x) DDRQ_DATA[3:0] <= {DDRQ_DLY[1:0], DDRQ[1:0]};
    ...
    reg [3:0] DATA_IN;
    always @(posedge clk) DATA_IN[3:0] <= DDRQ_DATA[3:0];

    reg [3:0] DATA_IN_DLY;
    always @(posedge clk) DATA_IN_DLY[3:0] <= {DATA_IN[3:0]};
