...
module fast_spi_rx_core #(
    ...
    always @(posedge BUS_CLK) begin
        if (BUS_RD) begin
            ...
            else if (BUS_ADD == 4) BUS_DATA_OUT <= DATA_SIZE_READ;
    ...
    always @(posedge SCLK) begin
        if (RST_SYNC) frame_cnt <= 0;
        else if (SEN_FINISH && CONF_EN_SYNC) frame_cnt <= frame_cnt + 1;
    end
    ...
    reg [4:0] bit_cnt;
    always @(posedge SCLK) begin
        if (RST_SYNC | SEN_START) bit_cnt <= 0;
        else if (cdc_fifo_write) bit_cnt <= 0;
        else if (SEN) bit_cnt <= bit_cnt + 1;
    end
    ...
    always @(posedge SCLK) begin
        if (RST_SYNC | SEN_FINISH) spi_data <= 0;
        else if (cdc_fifo_write) spi_data <= {{(DATA_SIZE - 1) {1'b0}}, SDI};
        else if (SEN) spi_data <= {spi_data[DATA_SIZE-2:0], SDI};
    end
