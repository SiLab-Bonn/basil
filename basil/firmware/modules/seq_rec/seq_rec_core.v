...
module seq_rec_core #(
    ...
    always @(posedge BUS_CLK) begin
        if (BUS_RD) begin
            ...
            else if (BUS_ADD == 1) BUS_DATA_OUT_REG <= {7'b0, CONF_READY};
            else if (BUS_ADD == 2) BUS_DATA_OUT_REG <= {7'b0, CONF_EN_EXT_START};
            else if (BUS_ADD == 8) BUS_DATA_OUT_REG <= MEM_BYTES[7:0];
            else if (BUS_ADD == 9) BUS_DATA_OUT_REG <= MEM_BYTES[15:8];
            else if (BUS_ADD == 10) BUS_DATA_OUT_REG <= MEM_BYTES[23:16];
            else if (BUS_ADD == 11) BUS_DATA_OUT_REG <= MEM_BYTES[31:24];
            else if (BUS_ADD < 16) BUS_DATA_OUT_REG <= BUS_STATUS_OUT;
        end
    end
    ...
    always @(posedge SEQ_CLK)
        ...
        else if (START_REC && DONE) out_bit_cnt <= 1;
        else if (out_bit_cnt == STOP_BIT) out_bit_cnt <= 0;
        else if (out_bit_cnt != 0) out_bit_cnt <= out_bit_cnt + 1;

    always @(posedge SEQ_CLK)
        if (RST_SYNC) DONE <= 1'b1;
        else if (START_REC && DONE) DONE <= 1'b0;
        else if (out_bit_cnt == 0) DONE <= 1'b1;
    ...
    always @(posedge BUS_CLK)
        if (RST) CONF_READY <= 1'b1;
        else if (START_REC_FLAG_BUS_CLK || CONF_START) CONF_READY <= 1'b0;
        else if (DONE_FLAG_BUS_CLK) CONF_READY <= 1'b1;
