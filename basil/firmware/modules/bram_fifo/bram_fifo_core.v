...
module bram_fifo_core #(
    ...
    always @(posedge BUS_CLK) begin
        if (BUS_RD) begin
            ...
            else if (BUS_ADD == 6) BUS_DATA_OUT <= CONF_SIZE_BYTE_BUF[23:16];
            else if (BUS_ADD == 7) BUS_DATA_OUT <= CONF_SIZE_BYTE_BUF[31:24];
    ...
    always @(posedge BUS_CLK) BUS_DATA_OUT_DATA <= FIFO_DATA_BUF;
    ...
    always @(posedge BUS_CLK) begin
        ...
        else if (((((FIFO_ALMOST_FULL_VALUE + 1) * DEPTH) >> 8) <= CONF_SIZE) ||
                 (FIFO_ALMOST_FULL_VALUE == 8'b0 && CONF_SIZE >= 0))
            FIFO_NEAR_FULL <= 1'b1;
        else if (((((FIFO_ALMOST_EMPTY_VALUE + 1) * DEPTH) >> 8) >= CONF_SIZE &&
