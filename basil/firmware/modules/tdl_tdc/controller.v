...
module controller #(
    ...
    always @(state, en_write_trigger_distance) begin
        ...
        case (state)
            ...
            FAL_EDGE: begin
                if (en_write_trigger_distance) mux_addr <= TRIG_IN;
                else mux_addr <= SIG_IN;
                counter_reset <= 1;
                counter_count <= 0;
            end
            CALIB: begin
    ...
    always @(posedge CLK) begin
        ...
        if (rst) begin
            ...
            case (state)
                ...
                RIS_EDGE: begin
                    if (hit) state <= FAL_EDGE;
                    else state <= state;
                end
                FAL_EDGE: begin
                    arm_flag_latch <= 0;
                    if (en_write_trigger_distance) state <= IDLE_TRIG;
                    else state <= IDLE;
                end
                CALIB: begin
                    if (~en_calib_mode) state <= IDLE_TRIG;
                    else if (hit) state <= CALIB_HIT;
                    else state <= CALIB;
                end
