module sample_deser(
	input wire CLK_FAST,
	input wire CLK_SLOW,
	input [dlyline_bits-1:0] sample_in,

	output reg [1:0 ] hit_status,
	output reg [fine_time_bits-1:0] medium_fine_time,
...
// indicate that the tdl is too fast for the sampling.
wire [2:0] miss_flags;
generate
...
always @(posedge CLK_SLOW) begin
	...
	medium_fine_time <= clk_ratio-1 - mux_address;

	if(|hit_flags) begin
		hit_status <= HIT;
	end 
	else if(|miss_flags) begin
		hit_status <= MISSED;
	end
	else begin
		hit_status <= IDLE;
