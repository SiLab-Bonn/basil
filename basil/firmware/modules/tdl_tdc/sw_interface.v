...
module tdc_sw_interface #(
    ...
    always @(posedge CLK) begin
        if (tdc_rst_short) tdc_rst_counter <= 3'b001;
        else if (tdc_rst_counter != 0) tdc_rst_counter <= tdc_rst_counter - 1;
    end
    ...
    always @(posedge BUS_CLK) begin
        if (bus_rst) begin
            status_regs[1] <= 8'h08;
            status_regs[2] <= 8'b0;
        end else if (ip_wr && ip_add == 1) status_regs[1] <= ip_data_in;
        else if (ip_wr && ip_add == 8) status_regs[2] <= ip_data_in;
    end
    ...
    always @(posedge BUS_CLK) begin
        if (ip_rd) begin
            if (ip_add == 0) ip_data_out <= VERSION;
            else if (ip_add == 1) ip_data_out <= status_regs[1];
            else if (ip_add == 2) ip_data_out <= event_cnt_buf[7:0];
            else if (ip_add == 3) ip_data_out <= event_cnt_buf_read[7:0];
            else if (ip_add == 4) ip_data_out <= event_cnt_buf_read[15:8];
            else if (ip_add == 5) ip_data_out <= event_cnt_buf_read[23:16];
            else if (ip_add == 6) ip_data_out <= fifo_lost_data_cnt_bus_clk;
            else if (ip_add == 7) ip_data_out <= tdc_miss_cnt_bus_clk;
            else ip_data_out <= 0;
        end
    end
