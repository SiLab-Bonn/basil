...
class NTCRegister(HardwareLayer):
    ...
    def init(self):
        ...
        if self._conf["NTC_type"] == "TDK_NTCG16H":
            ...
            self.R_RATIO = np.array(
                [221.9, 125.1, 73.38, 44.72, 28.16, 18.25, 12.14, 10.00, 8.283, 5.781, 4.120, 2.996, 2.214, 1.665, 1.451, 1.271, 0.9832, 0.7707, 0.6114, 0.5469]
            )
            self.B_CONST = np.array([3435.0] * len(self.R_RATIO))
            self.TEMP = np.array(
                [233.145, 243.15, 253.15, 263.15, 273.15, 283.15, 293.15, 298.15, 303.15, 313.15, 323.15, 333.15, 343.15, 353.15, 358.15, 363.15, 373.15, 383.15, 393.15, 398.15]
            )
