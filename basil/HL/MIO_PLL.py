...
class MIO_PLL(HardwareLayer):
   ...
   # constants
    ...
    def setFrequency(self, value):
        if (float(value) < 0.08 or float(value) > 200):
            print('[MIO_PLL ERROR] PLL frequency (' + str(value) + ' MHz) out of range. Allowed range: 80 kHz to 200 MHz')
            return False
    ...
    def _calculateParameters(self, fout):
        ...
        for self.q_counter in range(128):
            ...
            for self.div in range(2, 128):
                ...
                if (isinstance(q_d_f, int) and (q_d_f > (15*self.fref))):  # = f0 * p
                    if (int(q_d_f) % int(self.fref) == 0):      # p, q, and d found
                        ...
                        print ('PLL frequency set to ' + str(ftest) + ' MHz' + ' (VCO @ ' + str(fvco) + ' MHz)')                                           
                        #print ('PLL frequency set to ' + str(ftest) + ' MHz')                                           
                        return True
        print 'MIO_PLL: [ERROR] Could not find PLL parameters'
