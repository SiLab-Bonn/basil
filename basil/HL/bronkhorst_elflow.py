...
class Bronkhorst_ELFLOW(HardwareLayer):
    ...
    def write(self,cmd):
        ...
        if self.debug!=0:
            print "ELFLOW.write() %s"%repr(cmd_s)
        self._intf.write(cmd_s)
    def read(self):
        flg=0
        ret_s=self._intf.read()
        if self.debug!=0:
             print "ELFLOW.read() %s"%repr(ret_s)
        if len(ret_s)< 5 or ret_s[0]!=":" or ret_s[3:5]!=self.node:
            print "ELFLOW.read() format error ret=",repr(ret_s)
            return []
        ret_len=int(ret_s[1:3])
        if ret_len*2!=len(ret_s[3:-2]):
            print "ELFLOW.read() data lenth error ret=",repr(ret_s)
            return []
        ret=[]
    ...
    def set_setpoint(self,value):
        ...
        ret=self.read()
        if len(ret)!=3: 
            print "ELFLOW.set_setpoint() data lenth error ret=",ret
            return -1
        elif ret[0]==0 and ret[1]==0 and ret[2]==5:
            return 0
        else:
            print "ELFLOW.set_setpoint() ret error ret=",ret
            return -1
    def get_setpoint(self):
        ...
        if len(ret)!=5:
            ...
            print ret[0]==2, ret[1]==cmd[1], ret[2]==cmd[2]
            print "ELFLOW.set_setpoint() ret error ret=",ret
            return -1
    ...
    def set_valve_output(self,value):
        ...
        ret=self.read()
        if len(ret)!=3:
            print "ELFLOW.set_valve_output() data lenth error ret=",ret
            return -1
        elif ret[0]==0 and ret[1]==0 and ret[2]==7:
            return 0
        else:
            print ret[0]==2, ret[1]==cmd[1], ret[2]==cmd[2]
            print "ELFLOW.set_valve_output() ret error ret=",ret
            return -1
    def get_valve_output(self):
       ...
       if len(ret)!=7:
            ...
            print ret[0]==2, ret[1]==cmd[1], ret[2]==cmd[2]
            print "ELFLOW.get_valve_output() ret error ret=",ret
            return -1
    def set_controller_speed(self,value):
        ...
        ret=self.read()
        if len(ret)!=3:
            print "ELFLOW.set_controller_speed() data lenth error ret=",ret
            return -1
        elif ret[0]==0 and ret[1]==0 and ret[2]==7:
            return 0
        else:
            #print ret[0]==2, ret[1]==cmd[1], ret[2]==cmd[2]
            print "ELFLOW.set_controller_speed() ret error ret=",ret
            return -1
