...
class RegisterHardwareLayer(HardwareLayer):
    ...
    def get_configuration(self):
        ...
        for reg in self._registers.keys():
            ...
            if not ("properties" in descr and [i for i in write_only if i in descr["properties"]]) and not (
                "properties" in descr and [i for i in read_only if i in descr["properties"]]
            ):
                conf[reg] = self[reg]
        return conf
