...
from yaml import BaseLoader, load, scanner

from basil.HL.RegisterHardwareLayer import HardwareLayer

# SCPI command mandatory by IEEE 488.2
_scpi_ieee_488_2 = {"clear": "*CLS", "reset": "*RST", "trigger": "*TRG", "get_name": "*IDN?"}
...
class scpi(HardwareLayer):
    ...
    def init(self):
        ...
        device_desciption = os.path.join(
            os.path.dirname(__file__), self._init["device"].lower().replace(" ", "_") + ".yaml"
        )
        try:
    ...
    def __getattr__(self, name):
        ...
        def method(*args, **kwargs):
            ...
            try:
                command = (
                    self._scpi_commands["channel %s" % channel][name]
                    if channel is not None
                    else self._scpi_commands[name]
                )
            except Exception:
            ...
            if len(name_split) == 2 and name_split[0] == "set" and len(args) == 1 and not kwargs:
                ...
                raise ValueError(
                    "Invalid SCPI command %s for device %s with args=%s and kwargs=%s"
                    % (name, self.name, str(args), str(kwargs))
                )
