...
class Register(RegisterLayer):
    def __init__(self, driver, conf):
        super(Register, self).__init__(driver, conf)
        ...
        if 'fields' in self._conf:
            for field in self._conf['fields']:
                ...
                #    raise ValueError("Register " + self._conf['name'] + ":" + field['name'] + ": Invalid offset value. Specify LSB position.")

                if 'repeat' in field:
                    reg_list = []
                    for _ in range(field['repeat']):
                        reg = Register(None, field)
                        reg_list.append(reg)
