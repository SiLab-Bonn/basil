...
`elsif BASIL_SBUS
    ...
    assign BUS_DATA_OUT = BUS_DATA_OUT_1 | BUS_DATA_OUT_2 | BUS_DATA_OUT_3 | BUS_DATA_OUT_4 |
        BUS_DATA_OUT_5;
`endif
...
`endif
    ...
    wire EX_START_PULSE;
`ifndef BASIL_SBUS
    `define BASIL_TEST_PULSE_GEN pulse_gen
`else
    `define BASIL_TEST_PULSE_GEN pulse_gen_sbus
`endif
    `BASIL_TEST_PULSE_GEN #(
...
`ifndef BASIL_SBUS
    `define BASIL_TEST_SPI spi
`else
    `define BASIL_TEST_SPI spi_sbus
`endif
    `BASIL_TEST_SPI #(
...
`ifndef BASIL_SBUS
    `define BASIL_TEST_FAST_SPI_RX fast_spi_rx
`else
    `define BASIL_TEST_FAST_SPI_RX fast_spi_rx_sbus
`endif
    `BASIL_TEST_FAST_SPI_RX #(
...
`ifndef BASIL_SBUS
    `define BASIL_TEST_BRAM_FIFO bram_fifo
`else
    `define BASIL_TEST_BRAM_FIFO bram_fifo_sbus
`endif
    `BASIL_TEST_BRAM_FIFO #(
...
`ifdef BASIL_SBUS
    initial begin
        $display("Sbus modules not implemented yet");
        $finish;
    end
`endif
endmodule

`undef BASIL_TEST_BRAM_FIFO
`undef BASIL_TEST_FAST_SPI_RX
`undef BASIL_TEST_GPIO
`undef BASIL_TEST_PULSE_GEN
`undef BASIL_TEST_SPI
