...
class TestSimTimestamp(unittest.TestCase):
    def setUp(self):
        cocotb_compile_and_run([os.path.join(os.path.dirname(__file__), 'test_SimTimestamp640.v')])

        self.chip = Dut(cnfg_yaml)
        self.chip.init()

    def test_io(self):
        self.chip['timestamp640'].reset()
        self.chip['timestamp640']["ENABLE"] = 1
        ...
        # trigger timestamp
        self.chip['pulse_gen640'].set_delay(0x105)
        self.chip['pulse_gen640'].set_width(10)
        self.chip['pulse_gen640'].set_repeat(1)
        self.chip['pulse_gen640'].set_phase(0)
