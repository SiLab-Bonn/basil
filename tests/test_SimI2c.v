...
module i2c_slave_model (
    ...
    always @(posedge SCL) if (state == STATE_ADDR) REC_ADDRESS[7-bit_count] = SDA;

    reg [7:0] BYTE_DATA_IN;
    always @(posedge SCL) if (state == STATE_DATA_W) BYTE_DATA_IN[7-bit_count] = SDA;
...
module tb (
    ...
    i2c_slave_model ii2c_slave_model (
        .SDA(SDA),
        .SCL(SCL)
    );
