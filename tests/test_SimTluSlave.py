...
class TestSimTlu(unittest.TestCase):
    def setUp(self):
        cocotb_compile_and_run(
            [os.path.join(os.path.dirname(__file__), 'test_SimTluSlave.v')])
...
#         self.chip['TLU'].HANDSHAKE_BUSY_VETO_WAIT_CYCLES = 0
    ...
    def test_tlu_trigger_handshake_longdata(self):
        ...
        self.chip['TLU'].TRIGGER_VETO_SELECT = 255
        self.chip['TLU'].EN_TLU_VETO = 0
        self.chip['TLU'].DATA_FORMAT = 3
        self.chip['TLU'].TRIGGER_ENABLE = True
        self.chip['GPIO'].set_data([0x01])

        readings = 0
        while(self.chip['FIFO'].get_FIFO_INT_SIZE() < 4 * 3 and readings < 1000):
            readings += 1
        self.chip['GPIO'].set_data([0x00])  # disable trigger/TLU FSM
        self.chip['TLU'].TRIGGER_ENABLE = False

        self.assertEqual(self.chip['TLU'].DATA_FORMAT, 3)
        self.assertGreaterEqual(self.chip['FIFO'].get_FIFO_INT_SIZE(), 4 * 3)
        self.assertGreaterEqual(self.chip['TLU'].TRIGGER_COUNTER, 4)
        self.assertGreaterEqual(self.chip['TLU'].CURRENT_TLU_TRIGGER_NUMBER, 3)
        ...
        for i in range(4):
            self.assertEqual(data[3 * i + 2], 0x80000000 + i)
            self.assertEqual(data[3 * i + 1] & 0xE0000000, 0xC0000000)
            self.assertEqual(data[3 * i + 0] & 0xE0000000, 0xE0000000)
